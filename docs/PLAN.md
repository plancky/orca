...
docker-compose.yml   README.md  DESIGN.md  API.md  openapi.json  tests/
```
...
`openapi_examples` (esp. single/multi/hard-case examples on `/api/v1/query`). Interactive docs
at `/docs` (Swagger UI) and `/redoc` (ReDoc); `scripts/export_openapi.py` writes the spec to
repo-root `openapi.json` so the contract is version-controlled and diffable in PRs.
...
10+ sample queries with expected outputs (incl. edge cases) + OpenAPI spec (`openapi.json` + Swagger UI `/docs` + ReDoc `/redoc`) + docker-compose (Postgres+
...
8. **Docs + OpenAPI spec/docs (metadata, tags, examples, `export_openapi.py`, `/docs`, `/redoc`) + tests + sample-query outputs**.
